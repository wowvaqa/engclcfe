...
const SingleReinfConcreteErrHandler = () => {
  const {
    setModalInfoShow,
    setModalInfoText,
    setModalInputText,
  ...
  useEffect(() => {
    console.log("(SingleErroHandling) Reciving data for error handling: ");
  ...
  const handleErrors = () => {
    ...
    if (singleDimensioningData.name === "") {
      ...
      Number.isNaN(singleDimensioningData.no_of_bars) ||
      singleDimensioningData.no_of_bars <= 0
    ) {
...
export default SingleReinfConcreteErrHandler;
